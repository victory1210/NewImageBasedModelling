...
SampleIO::read_file (std::string const& filename,  SampleList* samples)
{
    ...
    core::TriangleMesh::Ptr mesh = core::geom::load_ply_mesh(filename);
    ...
    core::TriangleMesh::ValueList const& vvalues = mesh->get_vertex_values();
    if (!mesh->has_vertex_values())
        throw std::invalid_argument("Vertex scale missing!");

    core::TriangleMesh::ConfidenceList& vconfs = mesh->get_vertex_confidences();
    if (!mesh->has_vertex_confidences())
    {
        std::cout << "INFO: No confidences given, setting to 1." << std::endl;
        vconfs.resize(verts.size(), 1.0f);
    }
    core::TriangleMesh::ColorList& vcolors = mesh->get_vertex_colors();
...
SampleIO::open_file (std::string const& filename)
{
    ...
    this->stream.format = core::geom::PLY_UNKNOWN;
    while (true)
    {
        std::getline(this->stream.stream, line);
        ...
        if (tokens.size() == 3 && tokens[0] == "format")
        {
            ...
            if (tokens[1] == "ascii")
                this->stream.format = core::geom::PLY_ASCII;
            else if (tokens[1] == "binary_little_endian")
                this->stream.format = core::geom::PLY_BINARY_LE;
            else if (tokens[1] == "binary_big_endian")
                this->stream.format = core::geom::PLY_BINARY_BE;
            continue;
        }
        ...
        if (parsing_vertex_props && tokens[0] == "property")
        {
            if (tokens[1] == "float" && tokens[2] == "x")
                this->stream.props.push_back(core::geom::PLY_V_FLOAT_X);
            else if (tokens[1] == "float" && tokens[2] == "y")
                this->stream.props.push_back(core::geom::PLY_V_FLOAT_Y);
            else if (tokens[1] == "float" && tokens[2] == "z")
                this->stream.props.push_back(core::geom::PLY_V_FLOAT_Z);
            else if (tokens[1] == "float" && tokens[2] == "nx")
                this->stream.props.push_back(core::geom::PLY_V_FLOAT_NX);
            else if (tokens[1] == "float" && tokens[2] == "ny")
                this->stream.props.push_back(core::geom::PLY_V_FLOAT_NY);
            else if (tokens[1] == "float" && tokens[2] == "nz")
                this->stream.props.push_back(core::geom::PLY_V_FLOAT_NZ);
            else if (tokens[1] == "float" && tokens[2] == "confidence")
                this->stream.props.push_back(core::geom::PLY_V_FLOAT_CONF);
            else if (tokens[1] == "float" && tokens[2] == "value")
                this->stream.props.push_back(core::geom::PLY_V_FLOAT_VALUE);
            else if (tokens[1] == "float" && (tokens[2] == "red"
                || tokens[2] == "diffuse_red" || tokens[2] == "r"))
                this->stream.props.push_back(core::geom::PLY_V_FLOAT_R);
            else if (tokens[1] == "float" && (tokens[2] == "green"
                || tokens[2] == "diffuse_green" || tokens[2] == "g"))
                this->stream.props.push_back(core::geom::PLY_V_FLOAT_G);
            else if (tokens[1] == "float" && (tokens[2] == "blue"
                || tokens[2] == "diffuse_blue" || tokens[2] == "b"))
                this->stream.props.push_back(core::geom::PLY_V_FLOAT_B);
            else if (tokens[1] == "uchar" && (tokens[2] == "red"
                || tokens[2] == "diffuse_red" || tokens[2] == "r"))
                this->stream.props.push_back(core::geom::PLY_V_UINT8_R);
            else if (tokens[1] == "uchar" && (tokens[2] == "green"
                || tokens[2] == "diffuse_green" || tokens[2] == "g"))
                this->stream.props.push_back(core::geom::PLY_V_UINT8_G);
            else if (tokens[1] == "uchar" && (tokens[2] == "blue"
                || tokens[2] == "diffuse_blue" || tokens[2] == "b"))
                this->stream.props.push_back(core::geom::PLY_V_UINT8_B);
            else if (tokens[1] == "float")
                this->stream.props.push_back(core::geom::PLY_V_IGNORE_FLOAT);
            else if (tokens[1] == "uchar")
                this->stream.props.push_back(core::geom::PLY_V_IGNORE_UINT8);
            else if (tokens[1] == "int")
                this->stream.props.push_back(core::geom::PLY_V_IGNORE_UINT32);
            else
            {
                this->reset_stream_state();
    ...
    if (this->stream.format == core::geom::PLY_UNKNOWN)
    {
        this->reset_stream_state();
    ...
    {
        core::geom::PLYVertexProperty prop = this->stream.props[i];
        if (prop_table.size() < static_cast<std::size_t>(prop + 1))
            prop_table.resize(prop + 1, false);
        prop_table[prop] = true;
    }

    if (!prop_table[core::geom::PLY_V_FLOAT_X]
        || !prop_table[core::geom::PLY_V_FLOAT_Y]
        || !prop_table[core::geom::PLY_V_FLOAT_Z])
    {
        this->reset_stream_state();
        throw util::Exception("Missing sample coordinates");
    }
    if (!prop_table[core::geom::PLY_V_FLOAT_NX]
        || !prop_table[core::geom::PLY_V_FLOAT_NY]
        || !prop_table[core::geom::PLY_V_FLOAT_NZ])
    {
        this->reset_stream_state();
...
SampleIO::next_sample_intern (Sample* sample)
{
    ...
    for (std::size_t i = 0; i < this->stream.props.size(); ++i)
    {
        core::geom::PLYVertexProperty property = this->stream.props[i];
        switch (property)
        {
            case core::geom::PLY_V_FLOAT_X:
                this->ply_read(&sample->pos[0]);
                break;
            case core::geom::PLY_V_FLOAT_Y:
                this->ply_read(&sample->pos[1]);
                break;
            case core::geom::PLY_V_FLOAT_Z:
                this->ply_read(&sample->pos[2]);
                break;
            case core::geom::PLY_V_FLOAT_NX:
                this->ply_read(&sample->normal[0]);
                break;
            case core::geom::PLY_V_FLOAT_NY:
                this->ply_read(&sample->normal[1]);
                break;
            case core::geom::PLY_V_FLOAT_NZ:
                this->ply_read(&sample->normal[2]);
                break;
            case core::geom::PLY_V_FLOAT_R:
                this->ply_read(&sample->color[0]);
                break;
            case core::geom::PLY_V_FLOAT_G:
                this->ply_read(&sample->color[1]);
                break;
            case core::geom::PLY_V_FLOAT_B:
                this->ply_read(&sample->color[2]);
                break;
            case core::geom::PLY_V_UINT8_R:
                this->ply_read_convert(&sample->color[0]);
                break;
            case core::geom::PLY_V_UINT8_G:
                this->ply_read_convert(&sample->color[1]);
                break;
            case core::geom::PLY_V_UINT8_B:
                this->ply_read_convert(&sample->color[2]);
                break;
            case core::geom::PLY_V_FLOAT_VALUE:
                this->ply_read(&sample->scale);
                break;
            case core::geom::PLY_V_FLOAT_CONF:
                this->ply_read(&sample->confidence);
                break;
            case core::geom::PLY_V_IGNORE_FLOAT:
            {
                float dummy;
                this->ply_read(&dummy);
                break;
            }
            case core::geom::PLY_V_IGNORE_UINT8:
            {
                uint8_t dummy;
                this->ply_read(&dummy);
                break;
...
{
    *value = core::geom::ply_read_value<float>
...
{
    *value = core::geom::ply_read_value<uint8_t>
        (this->stream.stream, this->stream.format);
}
